class Vtree2:
    ...
    def read(filename):
        ...
        with open(filename,'r') as f:
            for line in f:
                ...
                if line.startswith('c'): continue
                    ...
                    node = Vtree2.internal_node(left,right)
                    left.parent = right.parent = node
                if node is not None:
                    node.id = node_id
                    nodes[node_id] = node
        ...
        return root, nodes #root CHANGED BY ALESSANDRO

    def save(self,filename):
        """Save vtree to file"""
        with open(filename,'w') as f:
            # write vtree file format
            f.write(Vtree2._vtree_file_format)
            # write vtree header
            node_count = 2*self.var_count - 1
            f.write('vtree %d\n' % node_count)
